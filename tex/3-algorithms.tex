...
\comment{HTML extraction is non trival}%
...
\comment{Assumptions when working with blogs}%
With web blogs as our target websites we made the following assumptionsthat will play a central role in our extraction procedure\Anote{assumptions}:
...
\comment{Per blog procedure, related work}%
...
\comment{Algorithm, inputs/output}%
...
\comment{General comment, embarrassingly parallel}%
...
\comment{Input from assumptions.}%
...
\comment{XPath selectors (id, else class, else path)}%
