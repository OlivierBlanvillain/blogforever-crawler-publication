...
In addition to the extraction of blog content that we have been discussing so far, the BlogForever crawler is also responsible for capturing blog visuals. Indeed, one of the requirements of the BlogForever platform is to provide snapshots of blogs visuals in order to preserve changes in appearance \cite[FR53]{blogforeverrequirements2009}. As presented in section \autoref{techniques}, our crawler embed a full web browser in the harvesting process. In addition (***rep) to the JavaScript rendering, the browser also take care of visu by screenschots

in order to support blogs that use JavaScript to download and display content.

*dis also takes case of screenshots*. Before opting for this we considered 3 other solutions: wget, wkhtmltopdf, oxpath. This subsection relates our evaluation of these tools for the specific purpose.
