...
Web feeds usually contain about 20 blog posts @oita2010, which is, in most cases, less than the total number of posts in blogs. Consequently, in order to effectively archive the entire content of a blog, it is necessary to download and process pages beyond the ones referenced in the web feed.
...
#extractionAlgo shows the generic procedure we use to build extraction rules. The idea is quite simple: for each pair of `(`*page, target*`)` input, compute, out of all possible extraction rules, the best one with respect to a certain `ScoreFunction`. The rule which is most frequently the *best rule* is then returned.
...
In our implementation, rules are queries in the XML Path Language (XPath). Consequently, standard libraries can be used to parse HTML pages and apply extraction rules, providing the `Apply` function used in #extractionAlgo. We experimented with 3 types of XPath queries: selection over the HTML `id` attribute, selection over the HTML `class` attribute and selection using a relative path in the HTML tree. `id` attributes are expected to be unique, and `class` attributes have showed in our experiments to have better consistency than relative paths over pages of a blog. For these reasons we opt to always favour `id` over `class`, and `class` over path, such that the \coderef{allrulesAlgo} function returns a single rule per node.
...
With concrete definitions of the functions \coderef{allrulesAlgo}, `Apply` and \coderef{similarityAlgo} (as the `ScoreFunction`), the definition of #extractionAlgo for article extraction is now complete. We can therefore proceed with a time complexity analysis.

First, let's assume that we have at our disposal a linear time HTML parser that constructs an appropriate data structure, indexing HTML nodes on their `id` and `class` attributes, effectively making `Apply` $\in$ \Oof{1}. As stated before, the outer loop splits the input into independent computations and each call to \coderef{allrulesAlgo} returns (in linear time) at most as many rules as the number of nodes in its *page* argument. Therefore, the body of the inner loop will be executed\ \Oof{n} times, where $n$ is the number of nodes. Because each extraction rule can return any subtree of the queried page, each call to \coderef{similarityAlgo} takes \Oof{n}, leading to an overall quadratic running time.
